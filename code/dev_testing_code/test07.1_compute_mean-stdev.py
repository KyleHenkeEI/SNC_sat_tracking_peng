...
def process_single_case(camera, tile, output_dir):
    ...
    output_filename = os.path.join(case_dir, 'mean_pixels.txt')
    np.savez(output_filename, mean)
    output_filename = os.path.join(case_dir, 'stdev_pixels.txt')
    np.savez(output_filename, stdev)
